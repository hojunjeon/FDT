...
주요 명령은 `gen`, `ingest`, `state`, `forecast`, `whatif`, `risk`, `goal`, `analyze`, `brief`, `chat`, `serve`, `eval`입니다. 구현 기준은 [`docs/03_FDT_설계.md`](docs/03_FDT_설계.md)와 [`리뷰 보완 계약`](docs/04_FDT_리뷰보완.md)입니다.

## 턴 로그

`brief`·`chat`·웹 채팅 턴은 `log/turns/YYYY-MM-DD/<surface>-<session_id>.jsonl`에 UTF-8 JSONL로 기록됩니다. 기본 경로는 저장소 안이며, 필요하면 `FDT_TURN_LOG_DIR` 환경변수로 덮어쓸 수 있습니다. 운영 지표는 다음처럼 생성합니다.
